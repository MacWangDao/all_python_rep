...
logger.add("lob122.log", rotation="100MB", encoding="utf-8", enqueue=True,
...
class LimitOrderBook:
    ...
    def calc_left_volume_on_price_vol(self, price, bs_type):
        orderdict = self.best_price_map.get(price)
        if orderdict is not None:
            return reduce(lambda x, value: x + value.left_volume if value.bs_type == bs_type else 0, orderdict.values(),
                          0)
    ...
    def match_best_price(self, ex, lclose_price: float = 0):
        ...
        # print(self.best_price_map)
        if self.best_price_map:
            bid_vol_dict = {}
            ask_vol_dict = {}
            max_vol_value = 0
            min_price_dict = {}
            all_prices = list(self.best_price_map.keys())
            all_prices.sort(reverse=True)
            # print(all_prices)
            # print(self.best_price_map.get(503.88))
            # od = self.best_price_map.get(503.88, {})
            # print(len(od.values()))
            #
            # for order in od.values():
            #     print("###################")
            #     print(order.left_volume)
            #     print(order.bs_type)
            #     print(order.kind)
            #     print(order.price)
            #     print(order.norder)
            #     print(order.place_datetime)

            # print(self.calc_left_volume_on_price_vol(503.88, 'B'))
            # # print(self.calc_left_volume_on_price_vol(503.88, 'S'))
            # ods = reduce(lambda x, y: x + y.left_volume if y.bs_type == "B" else x, od.values(), 0)
            # print(ods)
            # print(sum([order.left_volume for order in od.values() if order.bs_type == "B"]))
            for price in all_prices:
                bid_vol_value_sum = 0
                ask_vol_value_sum = 0
                for k, v in self.best_price_map.items():
                    if price <= k:
                        # bid
                        # orderdict = self.orderbook_on_prices[k]
                        # for order in orderdict.values():
                        #     print(order.left_volume)
                        # bid_vol_on_price = self.calc_left_volume_on_price_vol(k, 'B')
                        bid_vol_on_price = sum([order.left_volume for order in v.values() if order.bs_type == "B"])
                        if bid_vol_on_price > 0:
                            bid_vol_value_sum += bid_vol_on_price
                            # bid_vol_dict[price] = bid_vol_value_sum
                    if price >= k:
                        # ask
                        # ask_vol_on_price = self.calc_left_volume_on_price_vol(k, 'S')
                        ask_vol_on_price = sum([order.left_volume for order in v.values() if order.bs_type == "S"])
                        if ask_vol_on_price > 0:
                            ask_vol_value_sum += ask_vol_on_price
                            # ask_vol_dict[price] = ask_vol_value_sum
                # print(f"price:{price},S:{ask_vol_value_sum},B:{bid_vol_value_sum}")
                # print(bid_vol_value_sum)
                # print(ask_vol_value_sum)
                min_vol_value = min([bid_vol_value_sum, ask_vol_value_sum])
                # if price == 414.0:
                #     print([bid_vol_value_sum, ask_vol_value_sum])
                # min_price_dict[price] = min_vol_value

                if min_vol_value != 0:
                    # print(price, bid_vol_value_sum, ask_vol_value_sum)
                    min_price_dict[price] = [min_vol_value, abs(bid_vol_value_sum - ask_vol_value_sum)]
                # if bid_vol_value_sum != 0 and ask_vol_value_sum != 0:
                #     min_vol_value = min([bid_vol_value_sum, ask_vol_value_sum])
                #     min_price_dict[price] = min([bid_vol_value_sum, ask_vol_value_sum])
                # elif bid_vol_value_sum == 0 and ask_vol_value_sum != 0:
                #     min_vol_value = ask_vol_value_sum
                #     min_price_dict[price] = ask_vol_value_sum
                # elif ask_vol_value_sum == 0 and bid_vol_value_sum != 0:
                #     min_vol_value = bid_vol_value_sum
                #     min_price_dict[price] = bid_vol_value_sum
                # print(min_price_dict)
                # if min_vol_value > max_vol_value:
                #     max_vol_value = min_vol_value
            # for price, pvol in self.best_price_map.items():
            #     pvol_B = sum([order.left_volume for order in pvol.values() if order.bs_type == "B"])
            #     pvol_S = sum([order.left_volume for order in pvol.values() if order.bs_type == "S"])
            #
            #     bid_vol_value_sum = 0
            #     ask_vol_value_sum = 0
            #     for k, v in self.best_price_map.items():
            ...
            #
            #     min_vol_value = min([bid_vol_value_sum, ask_vol_value_sum])
            #     if min_vol_value >= pvol_B or min_vol_value >= pvol_S:
            #         if min_vol_value != 0:
            #             min_price_dict[price] = [min_vol_value, abs(bid_vol_value_sum - ask_vol_value_sum), pvol_B,
            #                                      pvol_S]

            if min_price_dict:
                # print(min_price_dict)
                last_price = max(min_price_dict.items(), key=lambda x: x[1][0])
                # print(last_price)
                if len(last_price) > 0:
                    vol_value_max = last_price[1][0]
                    # print(vol_value_max)
                    # multiple_values = []
                    # sh_multiple_values = []
                    # for k, v in min_price_dict.items():
                    #     if vol_value_max == v[0]:
                    #         # multiple_values.append(k)
                    #         # sh_multiple_values.append((k, v))
                    #         multiple_values.append((k, v))
                    multiple_values = [(k, v) for k, v in min_price_dict.items() if vol_value_max == v[0]]
                    # print(multiple_values)
                    last_price = min(multiple_values, key=lambda x: x[1][1])
                    if len(last_price) > 0:
                        multiple_values = [k for k, v in multiple_values if last_price[1][1] == v[1]]
                        if vol_value_max != 0:
                            if ex == 1 or ex == 'sh':
                ...
                # print(multiple_values)
                # print(sorted(multiple_values))
                # print(round(np.median(multiple_values), 2))
    ...
    def on_order_sh(self, order, is_last=False):
        ...
        if kind == 'A':  # new order
            ...
            order_vol_price_obj = Order(bs_type, kind, pd_dt, order_id, price, volume)
            ...
            newdata_ts_floor = pd_dt.floor(freq='S')
            # print(newdata_ts_floor)
            if is_hour_between(newdata_ts_floor, '09:15:00 +0800', '09:24:59 +0800'):
                # print(f"order:{newdata_ts_floor}")
                if self.best_price_map.get(price) is None:
                    self.best_price_map[price] = {}
                self.best_price_map[price][order_id] = order_vol_price_obj
            # update DOP
            ...
            if cancelorderobj is not None:
                ...
                newdata_ts_floor = pd_dt.floor(freq='S')
                if is_hour_between(newdata_ts_floor, '09:15:00 +0800', '09:19:59 +0800'):
                    best_cancelorderobj = self.best_price_map.get(price, {}).get(order_id)
                    if best_cancelorderobj:
                        best_cancelorderobj.cancel_order(pd_dt, volume)
                        price = best_cancelorderobj.price
                        order_id = best_cancelorderobj.norder
                        if (best_cancelorderobj.status == 'complete' or best_cancelorderobj.status == 'cancel') and \
                                best_cancelorderobj.left_volume <= 0:
                            del self.best_price_map[price][order_id]
                            if len(self.best_price_map[price]) == 0:
                                del self.best_price_map[price]
    ...
    def on_transaction_sh(self, trans, is_last=False):
        ...
        if ts_type == 0:  # 成交
            ...
            if askorderobj is not None:
                ...
                # newdata_ts_floor = pd_dt.floor(freq='S')
                # if is_hour_between(newdata_ts_floor, '09:15:00 +0800', '09:20:00 +0800'):
                #     print(22222222222222)
                #     del self.best_price_map[price][askorderobj]
                #     if len(self.best_price_map[price]) == 0:
                #         del self.best_price_map[price]
            if bidorderobj is not None:
                ...
                # if is_hour_between(newdata_ts_floor, '09:15:00 +0800', '09:20:00 +0800'):
                #     print(3333333333333)
                #     del self.best_price_map[price][bidorderobj]
                #     if len(self.best_price_map[price]) == 0:
                #         del self.best_price_map[price]
    ...
    def on_transaction_sz(self, trans, is_last=False):
        ...
        if ts_type == '0':  # 成交
            ...
            if cancelorderobj is not None:
                ...
                newdata_ts_floor = pd_dt.floor(freq='S')
                if is_hour_between(newdata_ts_floor, '09:15:00 +0800', '09:19:59 +0800'):
                    for order_id in [askorder, bidorder]:
                        best_cancelorderobj = self.best_price_map.get(price, {}).get(order_id)
                        if best_cancelorderobj:
                            best_cancelorderobj.cancel_order(pd_dt, volume)
                            price = best_cancelorderobj.price
...
def snapshot_generator_classic(lob, snapshot_ts):
    ...
    # print(df_on_lob)
    # df_on_lob.to_csv("df_on_lob.csv", index=False)
    # df_on_prices.to_csv("df_on_prices.csv", index=False)
    ...
    for msg in consumer:  # 迭代器，等待下一条消息
        ...
        if (ex_code == "1" and inst_code.startswith('6')) \
            ...
            if code == "321" or code == "94":
                # print(f'Running time: Seconds:{b - time.time()}')
                newdata_ts_floor = newdata_ts.floor(freq='S')

                # first_time = pd.to_datetime(f"{newdata_ts_floor.strftime('%Y-%m-%d')} 09:25 +0800", utc=True)
                # print(newdata_ts_floor)
                # print(first_time)
                # print(first_time == newdata_ts_floor)
                # print(newdata_ts_floor)
                # if is_hour_between(newdata_ts_floor, '09:15:00 +0800', '09:25:00 +0800'):
                #     print(f"match_best_price:{newdata_ts_floor}")
                ...
                if snapshot_ts_check_res is not None:
                    for s in snapshot_ts_check_res:
                        if is_hour_between(s, '09:15:00 +0800', '09:25:00 +0800'):
                            lob_map.get(key).match_best_price(int(ex_code),
                                                              lclose_price=lclose_price_lookup_tab.get(inst_code, 0))

                            # print(lob_map.get(key).last_price)
                        sp, sl, snapshot_ts_map[key] = snapshot_generator_classic(lob_map.get(key), s)
                        # print(sp)
                        # print(sl)
                        lob_map.get(key).new_ohlc_bar()
                        logger.info(f'took snapshot @ {snapshot_ts_map.get(key)} - {key}')
                        print(f'took snapshot @ {snapshot_ts_map.get(key)} - {key}')
                        f.write(str(snapshot_ts_map.get(key))+"\r")
                        f.flush()
                        ...
                        if snapshot_ts_map.get(key) is not None:
                            ...
                            sp_sl_to_influx(strex + '.' + instrument, sp, sl, influx_writer, bucket=lc.bucket,
                                            org=lc.org)
