...
logger.add("lob_v3.log", rotation="100MB", encoding="utf-8", enqueue=True,
...
def snapshot_generator_classic(lob, snapshot_ts):
    ...
    for k, v in lob.orderbook_on_prices.items():
        av = sum([order.left_volume for order in v.values() if order.bs_type == 'S'])
        # av = reduce(
        #     lambda x, value: x + value.left_volume if value.bs_type == 'S' else 0,
        #     v.values(), 0)
        asks_vol.append((k, av))
        asks_value.append((k, av * float(k)))
        bv = sum([order.left_volume for order in v.values() if order.bs_type == 'B'])
    ...
    for msg in consumer:  # 迭代器，等待下一条消息
        ...
        if (ex_code == "1" and inst_code.startswith('6')) \
            ...
            if code == "321" or code == "94":
                snapshot_ts_check_res = snapshot_time_checker(snapshot_ts_map.get(key), newdata_ts, lc.time_interval)
