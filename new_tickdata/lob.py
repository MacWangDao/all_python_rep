...
class LimitOrderBook:
    ...
    def update_prices_queue(self):
        ...
        self.bid_prices_queue_decr = []
        all_prices = []
        for k, v in sorted(self.orderbook_on_prices.items(), reverse=True):
            all_prices.append(k)
        # print(all_prices) # big->small

        for i in range(len(all_prices)):
            if all_prices[i] <= self.last_price:
                bid_vol_on_price = self.calc_left_volume_on_price(all_prices[i], 'B')
                # print('bid_vol_on_price %.2f  ->  %d'% (all_prices[i] ,bid_vol_on_price))
                if bid_vol_on_price > 0:  # 买量不是0
                    self.bid_prices_queue_decr.append(all_prices[i])
        all_prices.reverse()

        for i in range(len(all_prices)):
            if all_prices[i] >= self.last_price:
                ask_vol_on_price = self.calc_left_volume_on_price(all_prices[i], 'S')
                # print(self.ask_prices_queue_incr)
                # print('ask_vol_on_price %.2f  ->  %d'% (all_prices[i] ,ask_vol_on_price))
                if ask_vol_on_price > 0:  # 卖量不是0
                    self.ask_prices_queue_incr.append(all_prices[i])
    ...
    def match_best_price(self, ex, lclose_price: float = 0):
        ...
        if self.orderbook_on_prices:
            ...
            min_price_dict = {}
            if bid_vol_dict and ask_vol_dict:
                for price in all_prices:
                    if bid_vol_dict.get(price) and ask_vol_dict.get(price):
                        min_price_dict[price] = min([bid_vol_dict.get(price, 0), ask_vol_dict.get(price, 0)])
                if min_price_dict:
    ...
    def on_order(self, new_order, is_last=False):
        ...
        self.cached_orders.append(new_order)
        self.cached_orders = list(filter(lambda x: x is not None, self.cached_orders))

        for i in range(len(self.cached_orders)):
            order = self.cached_orders[i]
            processed = False
            exchange = order[3]
            if exchange == 1:  # sh
                processed = self.on_order_sh(order, is_last)
            elif exchange == 2:  # sz
                processed = self.on_order_sz(order, is_last)
            if processed == True:
                self.cached_orders[i] = None  # set to None to clear this order in the entry

        return len(self.cached_orders)
    ...
    def on_transaction_sh(self, trans, is_last=False):
        ...
        if ts_type == 0:  # 成交
            ...
            # print (askorder, bidorder)

            # print (ts_type , askorderobj,bidorderobj)
            # return True
    ...
    # date,time,tdftime,exchange,code,norder,nprice,nvolume,chorderkind,chfunctioncode,seqno,nOrderOriNo,nBizIndex
    # @logger.catch
    def on_order_sz(self, order, is_last=False):
        ...
        '''
        if bs_type == 'C': # 深交所order里没有撤单,在逐笔成交里
            if self.all_orders.get(order_id) is None:
                continue
            self.orderbook_on_prices[price][order_id].cancel_order(pd_dt,volume)
        '''
        return True

    # date,time,tdftime,exchange,code,nindex,lastprice,volume,turnover,nbsflag,chorderkind,chfunctioncode,naskorder,nbidorder,seqno
    # @logger.catch
    def on_transaction_sz(self, trans, is_last=False):
        ...
        if ts_type == '0':  # 成交
            ...
            if cancelorderobj is not None:
                ...
                self.try_remove_order(cancelorderobj)
                return True
        # return price, volume, turnover, bsflag
        # print (ts_type)
        return False
...
class DynamicOnPrice:  # DOP, dynamic on a price
    ...
    def __init__(self, price):
        ...
        # print(self.askorder_put_count_middle)
        ...
        self.trans_amt = 0.0
        '''
        # update by using transaction
        self.actbuy_count = 0
        self.actbuy_volume = 0
        self.actbuy_turnover = 0.0
        self.actsell_count = 0
        self.actsell_volume = 0
        self.actsell_turnover = 0.0
        self.actunknown_count = 0
        self.actunknown_volume = 0
        self.actunknown_turnover = 0.0
        
        # uptick/downtick/leveltick
        self.uptick_count = 0
        self.uptick_volume = 0
        self.uptick_turnover = 0.0
        self.downtick_count = 0
        self.downtick_volume = 0
        self.downtick_turnover = 0.0
        self.leveltick_count = 0
        self.leveltick_volume = 0
        self.leveltick_turnover = 0.0

        # update by using order
        self.ask_order_put_count = 0
        self.ask_order_put_volume = 0
        self.ask_order_cancel_count = 0
        self.ask_order_cancel_volume = 0

        self.bid_order_put_count = 0
        self.bid_order_put_volume = 0
        self.bid_order_cancel_count = 0
        self.bid_order_cancel_volume = 0
        '''
    ...
    def List_Data_Names():
        ...
        return lnames
        '''
        return ['actbuy_count', 'actbuy_volume', 'actbuy_turnover', \
                'actsell_count', 'actsell_volume', 'actsell_turnover', \
                'actunknown_count', 'actunknown_volume', 'actunknown_turnover', \
                
                'uptick_count', 'uptick_volume', 'uptick_turnover', \
                'downtick_count', 'downtick_volume', 'downtick_turnover', \
                'leveltick_count', 'leveltick_volume', 'leveltick_turnover', \
                
                'transaction_count', 'transaction_volume', 'transaction_turnover', \
                'ask_order_put_count', 'ask_order_put_volume', 'ask_order_cancel_count', 'ask_order_cancel_volume', \
                'bid_order_put_count', 'bid_order_put_volume', 'bid_order_cancel_count', 'bid_order_cancel_volume', \
                'update_ts']
        '''
    ...
    def list_data(self):
        ...
        for metric in DynamicOnPrice.MATRICS:
            for datetype in DynamicOnPrice.DATATYPES:
                ...
                for sizeclass in DynamicOnPrice.SIZECLASS:
                    # ldata.append( getattr(self,"%s_%s_%s"%(metric,datetype,sizeclass)) )
        ...
        '''
        return (self.price, [self.actbuy_count, self.actbuy_volume, self.actbuy_turnover, \
                             self.actsell_count, self.actsell_volume, self.actsell_turnover, \
                             self.actunknown_count, self.actunknown_volume, self.actunknown_turnover, \
                                 
                             self.uptick_count, self.uptick_volume, self.uptick_turnover, \
                             self.downtick_count, self.downtick_volume, self.downtick_turnover, \
                             self.leveltick_count, self.leveltick_volume, self.leveltick_turnover, \
                                 
                             self.transaction_count, self.transaction_volume, self.transaction_turnover, \
                             self.ask_order_put_count, self.ask_order_put_volume, self.ask_order_cancel_count,
                             self.ask_order_cancel_volume, \
                             self.bid_order_put_count, self.bid_order_put_volume, self.bid_order_cancel_count,
                             self.bid_order_cancel_volume, \
                             self.update_ts])
        '''
