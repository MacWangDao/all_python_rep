...
from app.influxdb.inf_to_redis import inf_redis_price, inf_redis_vol
    ...
    try:
        if combine_in.sto_id is not None:
            ...
            if res:
                ...
                redis_batid = request.app.state.redis_batid
                sk_name = f"sk-{combine_in.ssi_id}-{combine_in.ssid}-{combine_in.sto_id}"
                mapping = {
                    "stockcode": combine_in.stockcode,
                    "stockname": combine_in.stockname,
                    "exchangeid": sk_data_update["exchangeid"],
                    "price": combine_in.price,
                    "vol": combine_in.vol,
                    "bsdir": combine_in.bsdir,
                    "type": res.type,
                ...
                await redis_batid.hset(sk_name, mapping=mapping)
...
async def remove_all_combine_order_sk(
    ...
    try:
        if combine_in.ssi_id:
            ...
            keys = []
            for res in records:
                keys.append(f"sk-{res.ssiid}-{res.ssid}-{res.sto_id}")
            redis_batid = request.app.state.redis_batid
            if len(keys) > 0:
                await redis_batid.delete(*keys)
        return {"code": 20000, "data": {"status": "success"}, "message": "删除组合成功"}
    except Exception as e:
        logger.exception(e)
        raise HTTPException(status_code=400, detail=str(e))
...
async def remove_line_combine_order_sk(request: Request,
                                       *,
                                       db: Session = Depends(deps.get_db),
                                       current_user: User = Depends(
                                           deps.AccessPrivilegesPermissions(permission=5, error_msg="删除行数据")),
    ...
    try:
        if len(combine_list_in) > 0:
            ...
            for combine_in in combine_list_in:
                ...
                db.commit()
                keys.append(
                    f"sk-{combine_in.ssi_id}-{combine_in.ssid}-{combine_in.sto_id}"
                )
            redis_batid = request.app.state.redis_batid
            if len(keys) > 0:
                await redis_batid.delete(*keys)
...
async def go_order_by_id(request: Request,
    ...
    try:
        ...
        if len(records) > 0:
            ...
            aks = request.app.state.account_server.get(orders.account)
            if aks is None:
                return {"code": 40000, "data": orders, "message": "下单失败,交易系统未启动."}
    ...
    try:
        ...
        if len(order_list) > 0:
            ...
            for order in order_list:
                ...
                sto_id = order.sto_id
                ssi_id = order.ssi_id
                sk_name = f"sk-{ssi_id}-{ssid}-{sto_id}"
                sk_mapping = await redis_batid.hgetall(sk_name)
                if sk_mapping.get("stockcode") is not None and sk_mapping.get("type") is not None:
                    data_list = [
                        {
                            "ssid": ssid,
                            "vol": int(sk_mapping.get("vol")), "period": int(sk_mapping.get("period")),
                            "step": int(sk_mapping.get("step")),
                            "cancel": int(sk_mapping.get("cancel")), "type": int(sk_mapping.get("type")),
                            "stockcode": sk_mapping.get("stockcode"),
                            "price": float(sk_mapping.get("price")), "bsdir": int(sk_mapping.get("bsdir")),
                            "exchangeid": int(sk_mapping.get("exchangeid")),
                            "limit": int(sk_mapping.get("limit"))
                        }
                    ]
                else:
                    records = db.query(Strategy_Stock_Info.ssname,
                    ...
                    if len(records) > 0:
                        ...
                        return {"code": 40000, "data": orders, "message": "下单失败,没有记录."}
                    # backgroundtasks.add_task(inf_redis_price_vol)
                aks = request.app.state.account_server.get(orders.account)
                if aks is None:
                    return {"code": 40000, "data": orders, "message": "下单失败,交易系统未启动."}
                for res in data_list:
                    await redis_batid.incr("batid", amount=1)
                    batid = await redis_batid.mget("batid")
                    batid = int(batid[0])
                    kafka_data = {
                        "batid": batid,
                        "userid": userid,
                ...
                # print(job)
            return {"code": 20000, "data": res_data, "message": "下单成功"}
        else:
            return {"code": 40000, "data": orders, "message": "下单失败,没有记录."}
...
@router.post("/order_web", response_model=schemas.Msg, status_code=200, description="下单", summary="下单")
async def go_order_web(request: Request,
                       *,
                       db: Session = Depends(deps.get_db),
                       backgroundtasks: BackgroundTasks,
                       current_user: User = Depends(deps.AccessPrivilegesPermissions(permission=2, error_msg="下单")),
...
async def user_admin_list(
    ...
    try:
        ...
        accountinfos = db.query(AccountInfo).filter(AccountInfo.userid_id == userid,
                                                    AccountInfo.uname == uname, AccountInfo.status == 1).all()
        for acc in accountinfos:
            if acc.hostinfo.status:
                data_list.append(
                    {
                        "usernum": acc.usernum,
                        "account": acc.account,
        ...
        return {"code": 20000, "data": data_list, "message": "查询数据成功"}
    except Exception as e:
        logger.exception(e)
        raise HTTPException(status_code=400, detail=str(e))
    ...
    try:
        ...
        if current_user.userid:
            if user_order.qtype in [13, 15]:
                aks = request.app.state.account_server.get(user_order.account)
                if aks is None:
                    return {"code": 40000, "data": user_order, "message": "下单失败,交易系统未启动."}
