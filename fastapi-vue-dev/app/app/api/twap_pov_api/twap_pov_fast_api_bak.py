...
async def twap_pov_query(request: Request, qtype: int = 13, account: str = None, securityid: str = "",
    ...
    try:
        ...
        request.app.state.aks.hold_stock.clear()
        request.app.state.aks.total_asset.clear()
        request.app.state.aks.success_order.clear()
        if qtype == 13:
            ...
            asyncio.ensure_future(producer.send(req_pipe, key="user-query", value=query_order))
            await asyncio.sleep(1)
            return {"code": 20000, "data": request.app.state.aks.query_order, "message": "订单查询"}
        elif qtype == 9:
            ...
            await asyncio.sleep(1)
            return {"code": 20000, "data": request.app.state.aks.hold_stock, "message": "持仓查询"}
        elif qtype == 11:
            ...
            await asyncio.sleep(1)
            return {"code": 20000, "data": request.app.state.aks.total_asset, "message": "金额查询"}
        elif qtype == 15:
            ...
            await asyncio.sleep(1)
            return {"code": 20000, "data": request.app.state.aks.success_order, "message": "成交查询"}
...
@router.get("/cancal", response_model=schemas.Msg, description="撤单,ctype=0默认撤所有订单", summary="撤单,ctype=0默认撤所有订单")
async def twap_pov_cancal(request: Request, ctype: int = 0, account: str = None, securityid: str = "",
                          orderno: str = "",
                          sessionid: int = 0, current_user: User = Depends(
        ...
        if int(ctype) == 0:
            ...
            for order in orderinfo:
                ...
                if orderstatus in [0, 1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 13, 14]:
                    cancel_order = send_msg(type=7, securityid=securityid)
            ...
            logger.info(cancel_order)
            asyncio.ensure_future(producer.send(req_pipe, key="cancal_order", value=cancel_order))
        return {"code": 20000, "data": request.app.state.aks.cancal_order, "message": "撤单成功"}
    except Exception as e:
        logger.error(e)
        raise HTTPException(status_code=500, detail=str(e))
    finally:
