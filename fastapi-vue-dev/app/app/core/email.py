...
def send_registration_confirmed_email(client: EmailClient, user: User) -> None:
    send_data = MailGunSendData(
        from_=f"admin@{settings.email.MAILGUN_DOMAIN_NAME}",
        to=[str(user.useremail)],
    ...
    if response.status_code > 299:
        logger.error(
            f"Failed to send confirmation email for user: {user.useremail}, error: {response.error}"
        )
    else:
