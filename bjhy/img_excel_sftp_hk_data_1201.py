...
logger.add("./log/img_excel_sftp_hk_data_pro.log", rotation="100MB", encoding="utf-8", enqueue=True,
...
class Header(object):
    def __init__(self, tradedate_list, engine):
...
class Hksftpdata(object):
    ...
    def __init__(self, tradedate, auto=True):
        ...
        self.header = Header(self.tradedate_list, self.engine)
        self.run()
    ...
    def load_config(self):
        ...
        # logger.add(conf_fields.log_config.get("logpath"), rotation=conf_fields.log_config.get("rotation"),
        #            encoding=conf_fields.log_config.get("encoding"), enqueue=conf_fields.log_config.get("enqueue"),
        #            format=conf_fields.log_config.get("format"), retention=conf_fields.log_config.get("retention"))
        os.environ["NLS_LANG"] = "SIMPLIFIED CHINESE_CHINA.UTF8"
        dns = oracle.makedsn(conf_fields.oracle_config.get("host"), conf_fields.oracle_config.get("port"),
                             service_name=conf_fields.oracle_config.get("service_name"))
        self.engine = create_engine(
            f"oracle://{conf_fields.oracle_config.get('user')}:{conf_fields.oracle_config.get('password')}@" + dns,
    ...
    def history_A_stock_changenum(self):
        ...
        try:
            ...
            for date in self.tradedate_list:
                # sql = f'''select stockcode,stockname,industryname,TOTALSHARE,CIRCSKAMT,ROUND(holdnum /100 / CIRCSKAMT , 2),totalnu from (select stockcode,case substr(stockname, 1, 2) when 'DR' then( select distinct (stockname) from T_O_HK_BROKERHOLDNUM_NEW f where f.stockname not like 'DR%' and f.stockcode = t.stockcode and rownum = 1 ) when 'XD' then ( select distinct (stockname)from T_O_HK_BROKERHOLDNUM_NEW f where f.stockname not like 'DR%'and f.stockcode = t.stockcode and rownum = 1) else stockname end stockname,industryname,ROUND(g.TOTALSHARE / 10000,2) TOTALSHARE,ROUND(g.CIRCSKAMT / 10000,2) CIRCSKAMT, ROUND(sum(t.holdnum) /10000 , 2) holdnum,sum(changenum) totalnu from T_O_HK_BROKERHOLDNUM_NEW t ,TQ_SK_SHARESTRUCHG g ,TQ_SK_BASICINFO s where t.tradeday = to_date({date}, 'yyyy/mm/dd') and t.stockcode = s.symbol and g.compcode = s.compcode and  g.ENTRYDATE = ( select max(g.ENTRYDATE) from TQ_SK_SHARESTRUCHG g ,TQ_SK_BASICINFO f where g.compcode = f.compcode and f.symbol = s.symbol) and  g.begindate = ( select max(g.begindate) from TQ_SK_SHARESTRUCHG g ,TQ_SK_BASICINFO f where g.compcode = f.compcode and f.symbol = s.symbol) GROUP BY stockcode, stockname, industryname,ROUND(g.TOTALSHARE / 10000,2),ROUND(g.CIRCSKAMT / 10000,2))'''
                sql = f'''select t.stockcode, case substr(stockname, 1, 2) when 'DR' then (select distinct (stockname) 
                ...
                df_changenum_list = pd.read_sql(sql, self.engine, chunksize=50000)
                dflist = []
    ...
    def history_stock_ktcap(self):
        ...
        try:
            ...
            trade_df.set_index(["stockcode", 'stockname', "industryname"], drop=True, append=False, inplace=True)
            for date in self.tradedate_list:
                sql = f"""SELECT t.stockcode,t.stockname,t.industryname,t.sum_changenum ||','|| q.CIRCSKAMT ||',' || q.CIRCSKAMT * 10000 AS text 
    ...
    def history_A_stock_industry(self):
        ...
        try:
            ...
            for date in self.tradedate_list:
                ...
                df_industry_list = pd.read_sql(industry_totmktcap_sql, self.engine, chunksize=10000)
                dflist = []
        ...
        try:
            ...
            for date in self.tradedate_list:
                broker_sql = f'''
                    ...
                    '''
                df_broker_list = pd.read_sql(broker_sql, self.engine, chunksize=10000)
                dflist = []
    ...
    def get_tradedate_add_table(self, tradedate: str = None) -> List[str]:
        ...
        """
        try:
            tradedate_sql = "SELECT to_char(MAX(TRADEDAY),'yyyymmdd') AS TRADEDAY FROM T_HK_BROKERHOLDNUM_NEW"
            if tradedate:
                tradedate_sql = f"SELECT to_char(MAX(TRADEDAY),'yyyymmdd') AS TRADEDAY FROM T_HK_BROKERHOLDNUM_NEW WHERE TRADEDAY=to_date({tradedate}, 'yyyy/mm/dd')"
            df_tradedate = pd.read_sql(tradedate_sql, self.engine)
            tradedate_list = sorted(df_tradedate["tradeday"].tolist())
            return tradedate_list
        except Exception as e:
    ...
    def history_A_stock_ktcap(self):
        ...
        try:
            ...
            # and t.changenum !=0
            for date in self.tradedate_list:
                sql = f'''select a1.stockcode,a1.stockname,a1.industryname,ROUND(a2.TOTMKTCAP / 10000),ROUND(a2.NEGOTIABLEMV / 10000),a2.marketvalue,ROUND(a2.marketvalue / ROUND(a2.NEGOTIABLEMV / 10000) * 100,2),a1.mul_totmktcap  
    ...
    def history_A_stock_top(self):
        ...
        try:
            ...
            for date in self.tradedate_list:
                print(date)
                sql = f'''select tag,stockcode,stockname,industryname,total from 
    ...
    def history_A_stock_lv(self) -> pd.DataFrame:
        ...
        try:
            ...
            trade_df.set_index(["stockcode", 'stockname', "industryname"], drop=True, append=False, inplace=True)
            for date in self.tradedate_list:
                sql = f"""select  stockcode,case substr(stockname,1,2) when 'DR' then (select distinct(stockname) 
                from T_O_HK_BROKERHOLDNUM_NEW f where  f.stockname not like 'DR%' and f.stockcode = t.stockcode and rownum = 1) 
...
if __name__ == "__main__":
    hk = Hksftpdata("20230417", auto=False)
