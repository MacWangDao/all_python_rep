...
def save_df_data(data_json, date):
...
@logger.catch
def call_back_ths_industry(tradedate, task):
    if task.result() is None:
        ...
        if status in [200, 201]:
            try:
                ths_data = save_df_data(jdata, tradedate)
                ths_pgh_update(ths_data)
            except Exception as e:
                pass
        else:
...
@logger.catch
async def fetch_ths(session, url, headers, data, ip=None):
    try:
        await asyncio.sleep(0.5)
        async with session.post(url, data=data, headers=headers, verify_ssl=False, timeout=10, proxy=None) as response:
            return await response.json(), response.status
    except Exception as e:
        print(e)


async def network_loop_ths_industry(url, find_date):
    tasks = []
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, trust_env=True) as session:
            ...
            data = {
                "question": f"{find_date} 二级行业,涨幅,流通股",
                "perpage": "100",
                "page": "1",
            ...
            task = asyncio.ensure_future(fetch_ths(session, url, headers, data))
            task.add_done_callback(functools.partial(call_back_ths_industry, find_date))
            tasks.append(task)
...
async def main():
    ...
    task1 = asyncio.ensure_future(network_loop_ths_industry(url, find_date))
    await task1


@scheduler.scheduled_job("cron", day_of_week="0-4", hour="9", minute="27", second="5", id='task103', max_instances=3,
