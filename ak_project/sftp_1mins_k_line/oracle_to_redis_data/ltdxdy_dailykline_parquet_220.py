...
logger.add("ltdxdy_dailykline_parquet_220.log", rotation="100MB", encoding="utf-8", enqueue=True,
...
def dxy_parquent_run():
    ...
    for year in years[:]:
        in_csv_path = f"{home_path}/project/ak_project/sftp_1mins_k_line/in_csv/orgin/{year}"
        out_parquent_path = f"{home_path}/project/ak_project/sftp_1mins_k_line/parquet/{year}"
...
def many_merge_file_to_parquet():
    ...
    root_path = f"{home_path}/project/ak_project/sftp_1mins_k_line/parquet"
...
if __name__ == "__main__":
    """
    ssh-keygen -t rsa
    ssh-copy-id -p 12220 -i ~/.ssh/id_rsa.pub percy@cronnex.com
    ps -aux | grep ltdxdy_dailykline_parquet_220.py | grep -v grep |awk '{print $2}'| xargs kill -9
    ps -aux | grep ltdxdy_dailykline_parquet_220.py | awk '{print $2}'|xargs kill -9
    ...
    rsync -avztW -e 'ssh -p12220' --ignore-existing --no-times ./parquet_20230420 percy@cronnex.com:/home/percy/works/hongyou/quote_data/quote_daily
    """
    home_path = "/data"
    write_data_redis()
    dxy_parquent_run()
